...
def _format_errors_brief(errors: List[str]) -> str:
    """Краткое описание ошибок для админского отчета."""
    if not errors:
        return "Нет ошибок"
...
def _generate_task_link(form_id: int, task_id: int) -> str:
    """Генерирует ссылку на задачу в Pyrus."""
...
async def run_slot_multi(slot: str) -> None:
    ...
    from collections import Counter as _GlobalCounter, defaultdict
    unknown_name_counts_all = _GlobalCounter()
    # Детальная информация о нерегистрированных преподавателях и их задачах
    teacher_tasks_detail = defaultdict(list)  # ФИО -> [(form_id, task_id, краткое_описание_ошибок)]
    ...
    for form_id in form_ids:
        try:
            ...
            # Копим ФИО непривязанных преподавателей глобально + детали задач
            for name, task_id, errors in ambiguous_to_admin:
                n = (name or "").strip()
                if n:
                    unknown_name_counts_all[n] += 1
                    # Добавляем детальную информацию о задаче
                    brief_errors = _format_errors_brief(errors)
                    teacher_tasks_detail[n].append((form_id, task_id, brief_errors))
            
            # Сохраняем краткий отчет по форме (БЕЗ ФИО для экономии места)
    ...
    if ADMIN_IDS:
        ...
        # Добавляем детальную сводку по нерегистрированным преподавателям
        if teacher_tasks_detail:
            report_lines += [
                "",
                "🎯 ДЕТАЛИ ПО НЕРЕГИСТРИРОВАННЫМ ПРЕПОДАВАТЕЛЯМ:",
                ""
            ]
            # Сортируем преподавателей по количеству задач (убывание)
            sorted_teachers = sorted(teacher_tasks_detail.items(), 
                                   key=lambda x: len(x[1]), reverse=True)
            
            for teacher_name, tasks in sorted_teachers[:15]:  # Показываем топ-15
                task_count = len(tasks)
                report_lines.append(f"{teacher_name} ({task_count} задач):")
                
                # Показываем все задачи в компактном формате
                for form_id, task_id, brief_errors in tasks:
                    task_link = _generate_task_link(form_id, task_id)
                    report_lines.append(f"\t🔗 {task_link}: {brief_errors}")
                
                report_lines.append("")  # Пустая строка между преподавателями
        report_text = "\n".join(report_lines)
